...
/*---------------------------------------------------------------------------------------*/
	...
	//sprintf(filename,"drone-%02d:%02d:%02d.csv",time_st->tm_hour,time_st->tm_min,time_st->tm_sec);

	sprintf(filename,"/home/pi/Desktop/data_file/drone.csv");
	...
	/*fp = fopen(filename,"w");	
		...
		if(fp==NULL){
	   		printf("%sファイルが開けません",fp);
		}
	fprintf(fp," ,");
	for(i = 0; i<149; i = i + 1){
		if(i<=147){
			fprintf(fp,"%d,",2400000+j);
		}
		else {
			fprintf(fp,"%d",2400000+j);
		}	
	...
	while (1) {
		...
		while (neturl != NULL && spectool_netcli_getpollfd(&sr) >= 0 &&
			...
			if ((ret = spectool_netcli_poll(&sr, errstr)) < 0) {
				...
				exit(1);fprintf(fp,"%02d:",time_st->tm_hour);
					fprintf(fp,"%02d:",time_st->tm_min);					
					fprintf(fp,"%02d:",time_st->tm_sec);
			}
...
/*---------------------------------------------------------------------------------------------------------------*/
					...
					fprintf(fp,"%02d:",time_st->tm_hour);
					fprintf(fp,"%02d:",time_st->tm_min);					
					fprintf(fp,"%02d:",time_st->tm_sec);
					fprintf(fp,"%06d,",myTime.tv_usec);
					...
					for (r = 0; r < sb->num_samples; r++) {	
						...
						fprintf(fp,"%d",SPECTOOL_RSSI_CONVERT(sb->amp_offset_mdbm, sb->amp_res_mdbm,sb->sample_data[r]));
						
						if(r<sb->num_samples-1){
							fprintf(fp,",");
						}
					}
					...
					if(count%200==0){
						...
						sprintf(text,"cp -f /home/pi/Desktop/data_file/drone.csv /home/pi/Desktop/data_file/drone-%02d:%02d:%02d.csv",time_st->tm_hour,time_st->tm_min,time_st->tm_sec);
						system(text);
						system("mv -f /home/pi/Desktop/data_file/drone.csv /home/pi/Desktop/data_file/log_latest.csv");
						system("python /home/pi/Desktop/data_file/data_client.py");
